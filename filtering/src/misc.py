...
def hsv_red_contours(args):
    ...
    mask1 = cv2.inRange(hsv, np.array([0, 110, 100]), np.array([10, 255, 255]))
    mask2 = cv2.inRange(hsv, np.array([170, 110, 100]), np.array([180, 255, 255]))
    ...
    areas = sorted(
        [(b, b[2] * b[3]) for b in bboxes if b[2] * b[3] >= 900], key=itemgetter(1)
    )
