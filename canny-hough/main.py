	...
	for y in range(max_radius, height - max_radius):
		for x in range(max_radius, width - max_radius):
			...
			if optimize:
				...
				idx = ((arr_r - min_radius) * height * width)[:, None] + (arr_a * width) + (arr_b)
				...
				group_idx = np.flatnonzero(np.concatenate(([True], idx[1:] != idx[:-1], [True])))
...
if __name__ == '__main__':
	...
	t1 = time.perf_counter()
	circles = hough(canny_image, min_radius=40, max_radius=60, dist_between=50, count=3, optimize=True)
	t2 = time.perf_counter()
